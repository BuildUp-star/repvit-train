...
# -*- coding: utf-8 -*-
"""
print_high_neg_pairs.py
-----------------------
Print ALL negative pairs (different groups) with cosine similarity > threshold.
Optionally save them to CSV.

Usage examples:
  python print_high_neg_pairs.py --csv dataset/00train_test/test.csv --base_dir dataset/00train_test --threshold 0.7
  python print_high_neg_pairs.py --csv dataset/00train_test/train.csv --checkpoint runs/repvit_embed_512_best.pt --threshold 0.75 --save_csv runs/high_neg_pairs.csv
...
def parse_args():
    ap = argparse.ArgumentParser(description="Print NEG pairs with cosine similarity above a threshold.")
    ...
    ap.add_argument("--threshold", default=0.7, type=float, help="NEG cosine similarity threshold")
    ap.add_argument("--save_csv", default="", type=str, help="optional path to save matching pairs")
    ap.add_argument("--max_print", default=2000, type=int, help="limit the number of printed pairs (for very large sets)")
...
def main():
    ...
    mask = (~Y) & (S > args.threshold)
    idxs = np.flatnonzero(mask)

    print(f"\n=== NEG pairs with cosine > {args.threshold} ===")
    print(f"count = {idxs.size}")
    to_print = idxs[:args.max_print]
    for g in to_print.tolist():
        i = iu[0][g]
        j = iu[1][g]
        print(f"sim={S[g]:.4f} | NEG | group[i]={groups[i]} | group[j]={groups[j]}\n"
              f"  a: {paths[i]}\n  b: {paths[j]}")
    if idxs.size > args.max_print:
        print(f"... (truncated, printed first {args.max_print} of {idxs.size})")
    ...
    if args.save_csv:
        ...
        with open(args.save_csv, "w", newline="", encoding="utf-8") as f:
            ...
            for g in idxs.tolist():
                i = iu[0][g]
                j = iu[1][g]
                w.writerow([paths[i], groups[i], paths[j], groups[j], f"{S[g]:.6f}"])
        print(f"\n[Saved] wrote {idxs.size} pairs to {args.save_csv}")
